...
#sélection des métriques de la fonction écologique 
UREC_RegProd = st_read('C:/Users/Hp/Documents/STAGE_SHERBROOKE/results_normalization/UREC_merge_norm.SQLite')
#UREC_RegProd = UREC_RegProd %>% select(c('srfc_pF_nrm', 'srfc_pG_nrm', 'srfc_pU_nrm', 'srfc_pA_nrm', 'HEmPiedBer_nrm'))
UREC_RegProd = UREC_RegProd[, c('id_uea', 'id_rive', 'srfc_pf_nrm', 'srfc_pg_nrm', 'srfc_pu_nrm', 'srfc_pa_nrm', 'hempiedber_nrm')]
...
Correlation_matrix <- function(df, var2) {
  df = as.data.frame(df)
  df_sub = df %>% select(-c('id_uea', 'id_rive', 'GEOMETRY')) #subset dataframe to only select numeric data
  res <-
...
UREC_RegProd_pca = st_read('C:/Users/Hp/Documents/STAGE_SHERBROOKE/results_normalization/UREC_merge_norm.SQLite')
UREC_RegProd_pca = as.data.frame(UREC_RegProd_pca)
UREC_RegProd_pca = UREC_RegProd_pca[, c('srfc_pf_nrm', 'srfc_pg_nrm', 'srfc_pu_nrm', 'srfc_pa_nrm', 'hempiedber_nrm')]
...
pca = PCA_graph_function(UREC_RegProd_pca,'metrics_pca')
